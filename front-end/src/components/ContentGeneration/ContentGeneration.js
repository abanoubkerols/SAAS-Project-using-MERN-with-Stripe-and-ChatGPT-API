...
import { chatGptApi } from "../../apis/ChatGptApi/chatGptApi";
...
const BlogPostAIAssistant = () => {
  ...
  const mutation = useMutation({ mutationFn: chatGptApi })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-900 flex justify-center items-center p-6">
      <div className="bg-white rounded-xl shadow-xl overflow-hidden max-w-2xl w-full p-6">
        ...
        {mutation.isError && <StatusMessage type='error' message={mutation.data.response.error} />}


     <div className="flex">
        <div className="m-5"> 
            <span className="text-sm font-semibold bg-green-200 px-4 py-2 rounded-full">paln : {data.user.subscriptionPlan}</span>
        </div>

        <div className="m-5"> 
            <span className="text-sm font-semibold bg-green-200 px-4 py-2 rounded-full">credit : {data.user.apiRequestCount} / {" "} {data.user.monthlyRequestCount}
